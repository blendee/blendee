...
public class ReturningInserter<T> {
	...
	public ReturningInserter(TablePath path) {
		...
		var metadata = BlendeeManager.get().getMetadata();

		var pkMetadata = metadata.getPrimaryKeyMetadata(path);

		var columnNames = pkMetadata.getColumnNames();
		//"PK の項目が複数あります table=[" + path + "]"
		if (columnNames.length != 1) throw new IllegalStateException("multiple columns in PK of table=[" + path + "]");

		columnName = columnNames[0];

		var converter = ContextManager.get(BlendeeManager.class).getConfigure().getDataTypeConverter();
		var extractors = ContextManager.get(ValueExtractorsConfigure.class).getValueExtractors();

		for (var columnMetadata : metadata.getColumnMetadatas(path)) {
	...
	public T insertAndGetSequencePK(DataObject data) {
		var myPath = data.getPrimaryKey().getTablePath();
