		...
		try {
			annotations.forEach(a -> {
				roundEnv.getElementsAnnotatedWith(a).forEach(e -> {
					...
					param.put("PACKAGE", packageName.equals("") ? "" : ("package " + packageName + ";"));
					param.put("INTERFACE", className(packageName, e));
					param.put("METHODS", buildMetodsPart(infos));

					template = Formatter.format(template, param);

					TypeElement element = e.accept(new TypeVisitor(), null);

					String qualifiedName = element.getQualifiedName() + SQLProxyBuilder.METADATA_CLASS_SUFFIX;
					try {
						try (Writer writer = super.processingEnv.getFiler().createSourceFile(qualifiedName).openWriter()) {
